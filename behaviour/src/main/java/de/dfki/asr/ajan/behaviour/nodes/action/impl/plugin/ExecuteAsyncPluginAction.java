...
public class ExecuteAsyncPluginAction extends AbstractChainStep {

	public ExecuteAsyncPluginAction(final TaskStep next) {
		super(next);
	}
	...
	public Task.Status execute(final TaskContext context) {
		...
		ModelEvent event = context.get(ModelEvent.class);
		plugin.getRun().run(inputModel, event, action.getId(), action.getObject(), action.getUrl());
