...
public class ExecutePluginAction extends AbstractChainStep {

	public ExecutePluginAction(final TaskStep next) {
		super(next);
	}
	...
	public Task.Status execute(final TaskContext context) {
		...
		ResultModel output = plugin.getRun().run(inputModel, action.getObject(), action.getUrl());
		context.put(output);
