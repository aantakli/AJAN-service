...
public class TryFirstThenOnFailure extends AbstractChainStep {
	private final TaskStep nextStepOnFail;

	public TryFirstThenOnFailure(final TaskStep tryFirst, final TaskStep whenFirstFails) {
		super(tryFirst);
		nextStepOnFail = whenFirstFails;
	}

	@Override
	public Task.Status execute(final TaskContext context) {
		Status status = executeNext(context);
		if (status == Status.FAILED) {
			return nextStepOnFail.execute(context);
		} else {
			return status;
		}
	}
