...
4. **Triplestore:** eigenes **Embedded-Tomcat-Launcher-Modul**, das die offiziellen RDF4J-5.3.1-Server- und Workbench-WARs unter `/rdf4j` und `/workbench` deployed; gleiche CLI (`java -jar triplestore.jar --httpPort=8090`). *(Korrigiert nach Etappe 3, Task 2: **javax.servlet-Linie, nicht Jakarta** — die RDF4J-5.3.1-WARs sind `web-app 2.4`, `spring-webmvc 5.3.39`, `jstl 1.2` und damit auf javax.servlet aufgebaut; ein Jakarta-Tomcat (10/11) kann sie nicht deployen. Deshalb Embedded **Tomcat 9.0.x** (konkret 9.0.109). Der Wechsel auf die Jakarta-Linie ist erst möglich, wenn RDF4J selbst Jakarta-WARs liefert, und ist **kein** Bestandteil von Etappe 4; Tomcat 9 läuft auch unter Java 21, der Java-21-Bump in Etappe 4 erzwingt hier also keinen Container-Wechsel.)* Der Kontextpfad `/rdf4j` ist **normativ gesetzt** und bleibt der heutige (Kompatibilität mit bestehenden Konfigurationen und Start-Skripten); dass das offizielle `eclipse/rdf4j-workbench`-Image stattdessen `/rdf4j-server` verwendet (s. Spike A), betrifft nur das Image, nicht dieses Launcher-Modul — der Client bindet an eine konfigurierbare Basis-URL.
...
| triplestore | Neues Launcher-Modul: kleiner Main + Embedded **Tomcat 9.0.x** (javax.servlet-Linie, **nicht** Jakarta — korrigiert nach Etappe 3, s. Abschnitt 2.4), deployed RDF4J-5.3.1-Server- und Workbench-WARs; Ports/Pfade/CLI identisch. `tomcat8-maven-plugin` und tote Plugin-Repos entfernt. |
| RDF4J | **5.3.1** überall, eine Version im Parent verwaltet (verifiziert: Java-11-Bytecode, Class-Major 55). |
...
2. RDF4J-Client 3.6.3 → **5.3.1** im Produktcode inkl. der API-Anpassungen an `Repositories`/`RepositoryConnection`. **Reichweite:** Die Version ist zentral im Parent-POM verwaltet (`org.eclipse.rdf4j.version`), der Bump zieht deshalb zwingend mit: rdfbeans (gevendort), common, behaviour, functions **und das gesamte Pluginsystem** — 9 POMs deklarieren `rdf4j-model`/`rdf4j-repository-api` (`pluginsystem/pom.xml` + 8 Plugin-POMs), 75 Java-Dateien in **allen 10** Plugins importieren `org.eclipse.rdf4j.*` (OPCUA und ScriptExecutor erben die Dependency, ohne sie zu deklarieren). Etappe 3 ist damit keine Zwei-Modul-Etappe.
...
4. **Entscheidung zum RDF4J-Pin der E2E-Suite (getroffen):** `e2e/pom.xml` bleibt bewusst auf `rdf4j-rio-turtle`/`rdf4j-model` **3.6.3** (test-scope) — Auftraggeber-Entscheidung 2026-08-11. Die Suite testet ausschließlich von außen über HTTP und soll alle Migrationsetappen unverändert überleben; ein vom Produktcode entkoppelter Rio-Parser ist dafür das robustere Werkzeug, weil er die Antworten des Service parst statt seiner Interna. Der veraltete Kommentar „Same RDF4J line as the current project" wurde entsprechend korrigiert.
5. **SPIN-Entscheidung:** `rdf4j-sail-spin` existiert ab RDF4J 4.0 nicht mehr. Der `Inferencing`-Enum (`common/src/main/java/de/dfki/asr/ajan/common/TripleStoreManager.java`) behält alle vier Werte (`NONE`, `RDFS`, `SPIN`, `RDFS_SPIN`); die `SPIN`-/`RDFS_SPIN`-Zweige in `RDF4JTripleStoreManager` und `ExecutionBeliefBase` werfen jetzt `UnsupportedOperationException`. Laufzeitverhalten **unverändert**, weil `RDFAgentBuilder.java:96,152` und `ParameterAgentBuilder.java:81` `Inferencing.NONE` hart setzen — repo-weit konstruiert kein Aufrufer je `SPIN`/`RDFS_SPIN`.
5b. **Jackson-Auslassung (bewusst, gemessen):** Etappe 3 setzt **keinen** `<jackson.version>`-Override in `executionservice/pom.xml`, obwohl Boot 1.3.5 Jackson auf 2.6.6 pinnt. Beleg (Gate E, `docs/superpowers/specs/2026-07-11-spike-results.md`): alle vom Produkt genutzten RDF4J-Pfade tragen unter 2.6.6 — Repository-Verwaltung, `Repositories.graphQuery`, Rio Turtle/JSON-LD, und der von `BehaviorSelectQuery` tatsächlich genutzte Default-SELECT-Pfad (SPARQL/XML, nicht JSON — `SPARQLProtocolSession.preferredTQRFormat` ist XML). Nur ein erzwungener SPARQL-Results-JSON-Zugriff (`AbstractSPARQLJSONWriter`/`-Parser`) verlangt Jackson ≥2.10 und bricht sonst mit `NoClassDefFoundError: com/fasterxml/jackson/core/JsonFactoryBuilder`; dieses Format fordert der Produktcode nirgends an (repo-weite Suche: 0 Treffer auf `TupleQueryResultFormat.JSON`/`SPARQLResultsJSON*`). Abgesichert durch `SparqlJsonGuardTest` in `common`, der beim Build fehlschlägt, sobald künftiger Code dieses Format anfordert (Guard scannt `common`/`behaviour`/`functions`/`executionservice`/`pluginsystem`, Positivkontrolle auf ≥100 gescannte Dateien). **Benanntes Restrisiko:** fordert künftiger Code SPARQL-Results-JSON an, bricht es zur Laufzeit, nicht zur Build-Zeit, falls der Guard-Scan diesen Code nicht erreicht (die Untergrenze ist eine globale Summe über alle fünf Module, nicht pro Modul — ein Totalausfall eines einzelnen großen Moduls bei sonst intaktem Scan würde nicht auffallen). Mit dem Boot-4-Sprung in Etappe 4 erledigt sich die Frage ohnehin, weil Boot 4 eine moderne Jackson-Linie mitbringt.

Nicht in Etappe 3: alles Nicht-RDF4J am Pluginsystem (pf4j-Bump, Jakarta, Java 21) — das bleibt Etappe 4.

Die Etappe bleibt auf **Java 11** und Boot 1.3. Die Java-Version **steht dem nicht entgegen** (RDF4J 5.x verlangt minimal Java 11) — das ist aber nur die notwendige, nicht die hinreichende Bedingung. Zwei Punkte waren **nicht durch einen Spike belegt** und von Plan 2 zuerst zu klären: (a) Verträglichkeit der RDF4J-5-Transitiven (Jackson, httpclient, slf4j) mit dem Boot-1.3-Dependency-Management; (b) die konkrete Embedded-Tomcat-Version, die unter Java 11 läuft (Tomcat 11 verlangt Java 17). **Scheitert (a), wird Etappe 3 doch mit Etappe 4 zusammengelegt** — diese Rückfallebene ist vom Auftraggeber mitbeschlossen. **Beide Punkte sind jetzt geklärt** (Plan 2, Tasks 1 und 2; die Rückfallebene wurde nicht gezogen): (a) durch Gate E — grün, mit genau einem Override-Kandidaten (`jackson.version`), den der Auftraggeber durch den in Punkt 5b beschriebenen Build-Guard ersetzen ließ statt ihn zu setzen; (b) Embedded Tomcat 9.0.109 (javax.servlet-Linie, s. Abschnitt 2.4 und Abschnitt 3).
...
Dockerfile auf Temurin 21; CI-Workflow angepasst; README/Start-Skripte aktualisiert; ausgeschlossenen `BeliefUpdateStallTest` neu bewerten (fixen oder dokumentiert draußen lassen); restliche Dependency-Updates. **Offener Punkt aus Etappe 3:** `.github/workflows/docker-image.yml` (Zeilen 42, 173, 174) zeigt weiterhin auf `triplestore-0.1-war-exec.jar` — das Launcher-Modul aus Etappe 3 erzeugt stattdessen `triplestore-0.1.jar`. Der Workflow wurde in Etappe 3 bewusst nicht angefasst (Global Constraint des Plans: `docker-image.yml` bleibt während des Epics unberührt, s. Abschnitt 5); hier auf `triplestore-0.1.jar` nachzuziehen.
...
2. **RDF4J 3.6 → 5 API-Bruch** in rdfbeans/common/behaviour/functions und im gesamten Pluginsystem — **eingetreten und in Etappe 3 abgearbeitet.** Konkrete Bruchstellen, mit Fundstellen: `org.eclipse.rdf4j.queryrender.builder.*` (`QueryBuilderFactory`) entfernt, ersetzt durch direkte SPARQL-Algebra-Auswertung über `SailConnection.evaluate` (`common/src/main/java/de/dfki/asr/ajan/common/SPARQLUtil.java`, Task 3); `SailQueryPreparer` entfernt, gleicher Ersatz; `CloseableIteration<E>` verlor den zweiten Typparameter und erweitert jetzt `java.util.Iterator<E>` (rdfbeans, `RDFBeanManager.getAll()`, Task 4); `Repository.initialize()` durch `init()` ersetzt an mehreren Stellen, u.a. `StateLoader.java:108`; `rdf4j-sail-spin` seit 4.0 nicht mehr verfügbar (s. Etappe 3, Punkt 5). Abgesichert: 13 neue Charakterisierungstests in `common` (`SPARQLUtilQueryTest` u.a.), das committete Äquivalenz-Harness `spikes/etappe3-query-equivalence/` (10/10 bestanden, OLD-vs-NEW gegen echtes RDF4J 3.6.3), rdfbeans 56/56 Tests, Plugin-Seite über `PluginLoadIT`. Abnahme erreicht: `mvn install` 21/21 Module, E2E-Suite 16/16.
3. **RDF4J-REST-Protokoll** alt-Client ↔ 5er-Server — **eingetreten** (Spike A: 10/16 E2E rot, Bruch bei SPARQL-GraphQuery, reines Repository-Listing funktioniert). Die hier vorregistrierte Konsequenz („wandert in den Kern-Sprung") wurde **bewusst überschrieben und vom Auftraggeber bestätigt** (2026-07-31): stattdessen wird Etappe 3 um den Client-Umstieg erweitert, um den RDF4J-Bruch vom Boot-4-/Java-21-Sprung isoliert diagnostizierbar zu halten (s. Etappe 3). Damit wird Risiko 2 zeitlich vorgezogen. Rückfallebene, falls RDF4J 5 unter Boot 1.3 transitiv nicht trägt: Etappe 3 und 4 doch zusammenlegen. (Heute läuft bereits gemischt: 3.6.3-Client gegen 4.0.0-Server-WAR.) **Erledigt:** Etappe 3 ist grün abgeschlossen (`mvn install` 21/21, E2E 16/16, CI-Lauf 31506406844); die Rückfallebene (Etappe 3 und 4 zusammenlegen) wurde **nicht** gezogen — Gate E lieferte GREEN mit genau einem Override-Kandidaten (`jackson.version`), den der Auftraggeber durch einen Build-Guard ersetzen ließ statt ihn zu setzen (s. Etappe 3, Punkt 5b).
