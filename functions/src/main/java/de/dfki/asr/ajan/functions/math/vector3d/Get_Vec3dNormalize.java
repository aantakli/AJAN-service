 ...
 * DFKI)
 */
public class Get_Vec3dNormalize implements Function {
	...
	public Value evaluate(final ValueFactory valueFactory, final Value... args) throws ValueExprEvaluationException {
		if (args.length != 1) {
			throw new ValueExprEvaluationException(
					"normalize function requires" + "exactly 1 argument, got "
							+ args.length);
		}
		...
		Vector3D vec1 = Vector3dUtil.getVector3D(((Literal) arg).stringValue());

		return valueFactory.createLiteral(vec1.normalize().toString());
	}
}
