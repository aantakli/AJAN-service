	...
	public Value evaluate(final ValueFactory valueFactory, final Value... args) throws ValueExprEvaluationException {
		if (args.length != 2) {
			throw new ValueExprEvaluationException(
					"cross function requires" + "exactly 2 argument, got "
							+ args.length);
		}
		...
		return valueFactory.createLiteral(Vector3D.crossProduct(vec1, vec2).toString());
	}
}
