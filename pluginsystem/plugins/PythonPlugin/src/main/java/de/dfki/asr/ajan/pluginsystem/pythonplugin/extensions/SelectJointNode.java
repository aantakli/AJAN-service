...
public class SelectJointNode extends AbstractTDBLeafTask implements NodeExtension {
  ...
  public String toString() {
    return "SelectJointNode (" + label + ")";
  }

  @Override
  public Resource getType() {
    return vf.createIRI("http://www.ajan.de/behavior/python-ns#SelectJointNode");
  }

  @Override
  public NodeStatus executeLeaf() {
    LOG = this.getObject().getLogger();
    LOG.info(this.getClass(), "================= Test" + getTargetJoint() + "=================");
    try {
  ...
  private NodeStatus runPythonScript() throws PythonException {
    LOG.info(this.getClass(), "Executing Python script for target joint: " + getTargetJoint());

    PythonPlugin.getPowerShellManager()
        .executeCommand(
            "python .\\main.py --origin-id= --joint-id "
                + getTargetJoint()
                + " --call select_joint --url \"opc.tcp://localhost:40451\"");
    return new NodeStatus(
