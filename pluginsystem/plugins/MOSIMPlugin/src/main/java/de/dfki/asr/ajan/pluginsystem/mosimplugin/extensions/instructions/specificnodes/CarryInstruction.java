...
@Extension
@RDFBean("bt-mosim:CarryBinding")
public class CarryInstruction extends AbstractAsyncInstruction {
	...
	@Getter @Setter
    private String mmu = "Object/Carry";
    ...
    @Override
    public IRI getType() {
        return vf.createIRI("http://www.ajan.de/behavior/mosim-ns#CarryBinding"); // Is not counted either
    }

	@Override
	public IRI getActionType() {
		return vf.createIRI("http://www.ajan.de/behavior/mosim-ns#Carry");
	}
    ...
    public String getLable() {
        return "CarryInstruction"; // to trigger this action AJAN places it under services:<getLable()>
    }

    @Override
    public String getDescription() {
        return "MOSIM MMU Carry Istruction";
    }

	@Override
    protected boolean performOperation(final MCoSimulationAccess.Client client, final String actionID) throws TException {
		carryID = actionID;
		MInstruction reach = MOSIMUtil.createMInstruction(instID, actionID, mmu, MOSIMUtil.createCarryProperties(targetID, hand), null, null, null);
	...
	public void setResponse(String id, Object response) {
		if (response instanceof MSimulationEvent) {
			...
			if (event.Type.equals("PositioningFinished") && event.Reference.equals(instID)) {
				ResultModel model = new ResultModel();
				Resource carry = vf.createBNode();
				model.add(avatar, vf.createIRI("http://www.dfki.de/mosim-ns#carried"), target);
				model.add(avatar, MOSIMVocabulary.HAS_INSTRUCTION, carry);
				model.add(carry, org.eclipse.rdf4j.model.vocabulary.RDF.TYPE, MOSIMVocabulary.INSTRUCTION);
				model.add(carry, MOSIMVocabulary.HAS_ACTION_ID, vf.createLiteral(instID));
				model.add(carry, MOSIMVocabulary.HAS_MMU, vf.createLiteral(mmu));
