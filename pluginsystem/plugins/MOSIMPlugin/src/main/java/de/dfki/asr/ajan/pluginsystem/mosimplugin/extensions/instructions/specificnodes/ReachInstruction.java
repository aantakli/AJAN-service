...
@Extension
@RDFBean("bt-mosim:ReachBinding")
public class ReachInstruction extends AbstractAsyncInstruction {
    ...
    @Override
    public IRI getType() {
        return vf.createIRI("http://www.ajan.de/behavior/mosim-ns#ReachBinding");
    }

	@Override
	public IRI getActionType() {
		return vf.createIRI("http://www.ajan.de/behavior/mosim-ns#Reach");
	}
    ...
    public String getLable() {
        return "ReachInstruction"; // to trigger this action AJAN places it under services:<getLable()>
    }

    @Override
    public String getDescription() {
        return "MOSIM MMU Reach Istruction";
    }

	@Override
    protected boolean performOperation(final MCoSimulationAccess.Client client, final String actionID) throws TException {
		MInstruction reach = MOSIMUtil.createMInstruction(instID, actionID, mmu, MOSIMUtil.createReachProperties(targetID, hand), null, null, null);
	...
	public void setResponse(String id, Object response) {
		if (response instanceof MSimulationEvent) {
			...
			if (event.Type.equals("end") && event.Reference.equals(instID)) {
				...
				model.add(avatar, vf.createIRI("http://www.dfki.de/mosim-ns#reached"), target);
